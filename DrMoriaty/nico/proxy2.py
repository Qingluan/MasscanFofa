...
if not os.path.exists(BP_SESSION_DIR):
    os.mkdir(BP_SESSION_DIR)
...
class ProxyRequestHandler(BaseHTTPRequestHandler):
        ...
        if res_body is not None:
            ...
            if content_type.startswith('application/json'):
                ...
                if len(encoding) ==2:
                    ...
                    encoding = 'utf-8'
                m = re.search(r'<title[^>]*>\s*([^<]+?)\s*</title>', res_body.decode(encoding, 'ignore'), re.I)
                if m:
                    h = HTMLParser()
    ...
    def save_handler(self, req, req_body, res, res_body):
        BPData.save_req_res(req, req_body, res, res_body)
